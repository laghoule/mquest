...
DRAW_TILE_VGA PROC
  ...
  MOV AX, [BP + 0]                ; AX = X position
  MOV BX, [BP + 2]                ; BX = Y position
  CALC_VGA_POSITION AX, BX        ; Calculate VGA position in DI

  POP BX                          ; Restore map type in BX
  MOV DX, [BP + 4]                ; Height of the tile (number of lines)

  ; --- draw the tile loop
  @dtv_draw_line:
    MOV CX, [BP + 6]              ; Width of the tile (number of pixels)
...
; Input: AX = tileset offset,
;        BX = map type (bg || fg)
;        DI = memory tile buffer
;        [BP + 0] = Map tile width
;        [BP + 2] = Map tile height
; Output: None
; Modified: None
...
DRAW_TILE_RAM PROC
  ...
  POP BX                          ; Restore map type in BX
  MOV DX, [BP + 2]                ; Height of the tile (number of lines)

  ; --- draw the tile loop
  @dtr_draw_line:
    MOV CX, [BP + 0]              ; Width of the tile (number of pixels)
    PUSH DI                       ; Save current line start
...
@dspr_next_layer:
  MOV AX, [BP + AX_SP_IDX + 4]      ; AX = X position
  MOV BX, [BP + BX_SP_IDX + 4]      ; BX = Y position
